...
namespace NUnit.Tests1
{
    ...
    public class SpreadsheetTest
    {
        ...
        // Beyond this point is for HW9

        // Remaining Work:
        // -Implement saving functionality (DONE)
        // -Implement loading functionality (DONE)

        // Potentially:
        // -Test saving and loading (will this be possible considering we are wrking with files?) (DONE)

        
        // It seems for testing we are running into the same issue as before on mocking file systems and such, will focus on smaller functions as done last time
        
        [Test]
        public void TestConvertToCellName()
        {
            // Could not test as we do not know how to test private static methods (there seems to be a way using Microsoft.VisualStudio but references would need to be added)
            // MSNuget package would also need to be added...
            // See: https://stackoverflow.com/questions/13602508/where-to-find-microsoft-visualstudio-testtools-unittesting-missing-dll
            // and https://stackoverflow.com/questions/29384639/unit-testing-in-c-sharp-of-private-static-method-accepting-other-private-static
            // PrivateType is included in this

            /*
            PrivateType

            int result = ().InvokeStatic("ConvertToCellName", new object[] { 0, 0 });

            Assert.That(, Is.EqualTo("A1"),"");
            */

            // Made a non static method version for testing purposes...
            // TDD commit skipped as test was though to be not possible...
            Spreadsheet testSheet = new Spreadsheet(5, 5);
            MethodInfo methodInfo = this.GetMethod("InstanceConvertToCellName", testSheet);

            string result = (string)methodInfo.Invoke(testSheet, new object[] { 0, 0 });
            Assert.That(result, Is.EqualTo("A1"), "Converting row, col to cell name failed");

            result = (string)methodInfo.Invoke(testSheet, new object[] { 4, 2 });
            Assert.That(result, Is.EqualTo("C5"), "Converting row, col to cell name failed, secondary case");
        }

        // No interactions with an XML file will be testable for us... This is the majority of the functionality for this assignment, reason for lack of many tests.

        // Also, ran into an unreplicatable error where xml said extra content on end of xml file when viewing it in browser, deleting the xml file fixed this
        // ^^UPDATE: resolved, was being caused by not overwritting when a larger xml was saved beforehand

        // Methods refactored for save to allow for testing...
        [Test]
        public void TestFormatXmlSave()
        {
            Spreadsheet testSheet = new Spreadsheet(5, 5);

            testSheet.GetCell(1, 1).Text = "5";

            MethodInfo methodInfo = this.GetMethod("FormatXmlSave", testSheet);

            XmlDocument result = (XmlDocument)methodInfo.Invoke(testSheet, new object[] { });
            Assert.That(result.GetElementsByTagName("spreadsheet")[0].SelectNodes("cell")[0].SelectNodes("text")[0].InnerText, Is.EqualTo("5"), "FormatXmlSave failed for formating an xml for single change spreadsheet");
        }

        // Smaller helper is tested to avoid any file interactions.
        [Test]
        public void TestLoadCell()
        {
            Spreadsheet testSheet = new Spreadsheet(5, 5);

            MethodInfo methodInfo = this.GetMethod("LoadCell", testSheet);
            methodInfo.Invoke(testSheet, new object[] { "B5", "123", "4294967295" });

            Assert.That(testSheet.GetCell(4, 1).Text, Is.EqualTo("123"), "LoadCell failed to set a cell's text");
            Assert.That(testSheet.GetCell(4, 1).BGColor, Is.EqualTo(4294967295), "LoadCell failed to set a cell's color");
        }

        // Beyond this point is for HW10

        // Remaining Work:
        // -Indicate reference to a cell out of range or not a cell name (DONE)
        // -Indicate reference to a cell that references itself (DONE)
        // -Indicate reference to a cell that has a circular reference (DONE)

        // Potentially:
        // -Switch exising blank cell reference to calculate as 0 (But didn't we make it unref variable on purpose?) (DONE with easy switch)
        // ****************************************************************************************
        // COMMENT OUT Throw UnassignedVariableException in AssignVariables() to go to default 0
        // ****************************************************************************************
        
        // What is expected when a single cell commits two or more of (unref, bad, ,self or circular)?
        
        [Test]
        public void TestOutOfRangeCell()
        {
            // Tests for detection of out of range cell for a cell referencing only an out of range cell.
            Spreadsheet testSheet = new Spreadsheet(5, 5);

            testSheet.GetCell(1, 1).Text = "=G6";

            Assert.That(testSheet.GetCell(1, 1).Value, Is.EqualTo("Bad Reference"), "Spreadsheet failed to detect out of range cell");

            // Tests for detection of out of range cell for a cell referencing a valid cell and one out of range.
            testSheet = new Spreadsheet(5, 5);

            testSheet.GetCell(0, 0).Text = "5";
            testSheet.GetCell(1, 1).Text = "=A1+G6";

            Assert.That(testSheet.GetCell(1, 1).Value, Is.EqualTo("Bad Reference"), "Spreadsheet failed to detect out of range cell with other vars");
        }

        [Test]
        public void TestBadCellName()
        {
            // Tests for detection of bad cell name reference for a cell referencing only an invalidly named cell.
            Spreadsheet testSheet = new Spreadsheet(5, 5);

            testSheet.GetCell(1, 1).Text = "=Ba";

            Assert.That(testSheet.GetCell(1, 1).Value, Is.EqualTo("Bad Reference"), "Spreadsheet failed to detect bad cell name reference simple case");

            // Tests for detection of bad cell name reference for a cell referencing only an invalidly named cell but correct char num format.
            testSheet = new Spreadsheet(5, 5);

            testSheet.GetCell(1, 1).Text = "=[4";

            Assert.That(testSheet.GetCell(1, 1).Value, Is.EqualTo("Bad Reference"), "Spreadsheet failed to detect bad cell name reference simple case");

            // Tests for detection of bad cell name reference for a cell referencing a valid cell and an invalidly named cell.
            testSheet = new Spreadsheet(5, 5);

            testSheet.GetCell(0, 0).Text = "5";
            testSheet.GetCell(1, 1).Text = "=A1+Ba";

            Assert.That(testSheet.GetCell(1, 1).Value, Is.EqualTo("Bad Reference"), "Spreadsheet failed to detect bad cell name reference with other vars");
        }

        [Test]
        public void TestSelfReference()
        {
            // Tests for detection of self reference for a cell referencing only itself.
            Spreadsheet testSheet = new Spreadsheet(5, 5);

            testSheet.GetCell(1, 1).Text = "=B2";

            Assert.That(testSheet.GetCell(1, 1).Value, Is.EqualTo("Self Reference"), "Spreadsheet failed to detect self reference simple case");

            // Tests for detection of self reference for a cell referencing a valid cell and itself.
            testSheet = new Spreadsheet(5, 5);

            testSheet.GetCell(0, 0).Text = "5";
            testSheet.GetCell(1, 1).Text = "=A1+B2";

            Assert.That(testSheet.GetCell(1, 1).Value, Is.EqualTo("Self Reference"), "Spreadsheet failed to detect self reference with other vars");
        }

        [Test]
        public void TestCircularReference()
        {
            // Tests for circular reference detection in two cells referencing eachother.
            Spreadsheet testSheet = new Spreadsheet(5, 5);

            testSheet.GetCell(1, 1).Text = "=C3";
            testSheet.GetCell(2, 2).Text = "=B2";

            Assert.That(testSheet.GetCell(2, 2).Value, Is.EqualTo("Circular Reference"), "Spreadsheet failed to detect circular reference simple case");

            // Tests for circular reference detection in three cell loop reference.
            testSheet = new Spreadsheet(5, 5);

            testSheet.GetCell(1, 1).Text = "=C3";
            testSheet.GetCell(2, 2).Text = "=D4";
            testSheet.GetCell(3, 3).Text = "=B2";

            Assert.That(testSheet.GetCell(3, 3).Value, Is.EqualTo("Circular Reference"), "Spreadsheet failed to detect circular reference 3 layer case");
        }
        

        // Purpose of clearing list: get rid of old variable reliaces so equation doesn't needlessly update

        // Every equation knows which cells that need to change when it changes
        // Previously, we cleared this list when cell is changed to get rid of old associations, THIS IS WRONG!!!!
        // It was only working because we were just expanding cells to update, unecissarily updating other cells (also was buggy (would nto work after 1st change))

        // Instead of clearing delegate list, we should remove that reliance from every cell it was added to


        // Now, delegates are correct but cause loop as one delegate finishes and goes to other delegate and so on
        // Solution: If a circular is detected, add the delegates only AFTER the text is set

        // But why is it in the loop in the first place?? Because delegates set during initial loop cell setup


        // WORKING! But only the cell that initially causes the circular will be set to Circular Reference
        
        // Update: now changes ALL in event of circular.
    }
}
