...
namespace Cpts321
{
    ...
    public class ExpressionTree
    {
        ...
        {
            if (IsLeftParenthesis(c) || IsRightParenthesis(c) || this.operatorNodeFactory.IsOperator(c))
            {
                return true;
            }
        ...
        private bool IsHigherPrecedence(char left, char right)
        {
        ...
        private bool IsLowerPrecedence(char left, char right)
        {
            return this.ComparePrecedence(left, right) < 0;
        }
        ...
        private bool IsLowerOrSamePrecedence(char left, char right)
        {
            return this.ComparePrecedence(left, right) <= 0;
        }
        ...
        private Node PopulateTree(string expression, char op)
        {
            for (int expressionIndex = expression.Length - 1; expressionIndex >= 0; expressionIndex--)
            {
                ...
                {
                    OperatorNode operatorNode = this.operatorNodeFactory.CreateOperatorNode(expression[expressionIndex]);
